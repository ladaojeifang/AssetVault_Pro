...
下列 **单条或批量资产导入** 接口均 **不支持** 在请求体传入 `tagIds`、`typeId` / `categoryIds`（与 Eagle 扩展「随导入带标签」不同）：

| 接口 | 说明 |
|------|------|
| `POST /asset/import` | 本机路径 |
| `POST /asset/importBatch` | 批量本机路径 |
| `POST /asset/importFolder` | 递归文件夹 |
| `POST /asset/importFromURL` | URL 直链 |
| `POST /asset/importFromURLBatch` | 批量 URL |
| `POST /asset/importFromDataUrl` | data URL（截图等） |
| `fullPageSession` / `articleBundleSession` / `pageVideoImport` 的 `finish` 或 job 完成 | 会话式 / 异步入库 |

**推荐两步：**

1. 导入成功，取得 `assetId`（或 `data.imported[]`）
2. 按需 `POST /tag/assign`、`POST /category/assign`（`typeId` 为 `__sys:{fileType}` 或**用户分类 uuid**；兼容单元素 `categoryIds`。每资产仅一种类型，赋值会**覆盖**原类型）

可选：导入时传 `targetFolderId` 将资产放入逻辑文件夹；**不能**通过导入 API 一次写入多个文件夹。

**例外 — 资料库整库合并：** `POST /library/importFromLibrary` 会按源库自动合并文件夹树、标签、**用户分类定义**及资产 `type_id`（按分类 **name** 映射），无需第二步。详见 [§5.2.1](#521-从其它资料库合并导入) 与 [library-import-from-library-spec.md](./library-import-from-library-spec.md)。
...
| 类型 | **分两步**：导入后 `POST /category/assign`（`typeId`；导入接口不传 `typeId` / `categoryIds`；见 [§3.7](#37-单资产导入与元数据标签--类型)） |
...
6. `POST /category/assign` — 可选，传入 `assetIds`、`typeId`（或单元素 `categoryIds`）

**批量保存：** 页面侧筛选后调用 `POST /asset/importFromURLBatch`，再对 `data.imported` 批量 `tag/assign` 与 `category/assign`。
...
| POST | `/api/v1/library/importFromLibrary` | 从其它资料库整库导入：archive→archive 或 catalog→catalog；合并文件夹、标签、**用户分类**；SHA-256 去重。catalog 见 [library-import-catalog-to-catalog-spec.md](./library-import-catalog-to-catalog-spec.md) |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/category/get` | 全部类型（用户分类 + 9 个系统格式，`kind` 区分） |
| GET | `/api/v1/category/info?id=` | 单个类型（含 `__sys:image` 等系统 id） |
| POST | `/api/v1/category/create` | 创建用户分类 |
| PATCH | `/api/v1/category/update` | 更新用户分类（系统项不可改） |
| DELETE | `/api/v1/category/delete` | 删除用户分类 |
| POST | `/api/v1/category/assign` | 设置资产有效类型（`typeId`；须导入后第二步，同 tag） |
| POST | `/api/v1/category/remove` | 将资产类型重置为检测格式（`__sys:{file_type}`） |
...
源库与当前活动库须 **同为 archive 或同为 catalog**（本机目录，含 `library.sqlite`）。请求**同步阻塞**直至完成；长时间导入请调大客户端超时。桌面端另有 IPC 进度事件 `library:import-progress`，HTTP 无进度 SSE。

```bash
curl -s -X POST http://127.0.0.1:41596/api/v1/library/importFromLibrary \
  -H "Content-Type: application/json" \
  -d "{\"sourceLibraryRoot\":\"G:\\\\Libraries\\\\MyRefs.library\"}"
```
...
```json
{
  ...
  "data": {
    ...
    "sourceDisplayName": "MyRefs",
    "sourceLibraryRoot": "G:\\Libraries\\MyRefs.library",
    "assetsAdded": 120,
    "assetsSkippedDuplicate": 15,
    "assetsFailed": 2,
    "foldersCreated": 3,
    "foldersMerged": 8,
    "tagsCreated": 5,
    ...
    "categoriesCreated": 2,
    "categoriesMerged": 4,
    "sourceLibraryTagName": "MyRefs",
    "errors": [
      { "sourceAssetId": "…", "filename": "missing.png", "reason": "items 目录不存在" }
    ]
  }
}
```
...
| 字段 | 说明 |
|------|------|
| `categoriesCreated` / `categoriesMerged` | 用户分类按 **name** 在目标库新建或复用 |
| `tagsCreated` / `tagsMerged` | 标签按 name 合并；另写入 `sourceLibraryTagName` |
| `importMode` | `catalog_to_catalog_same_machine` 时另有 `assetsAddedLocal`、`assetsAddedReferenced` 等 |

**常见错误：** `INVALID_SOURCE_MODE`（模式组合不支持）、`SAME_LIBRARY`、`SOURCE_NOT_FOUND`、`LIBRARY_BUSY`（已有导入任务）。

规格全文：[library-import-from-library-spec.md](./library-import-from-library-spec.md)。
...
| `search` | 搜索关键词（多词 AND） |
| `folderId` | 逻辑文件夹 ID |
| `fileType` | 按 `file_type` 筛选（兼容参数，等价于 `typeFilters=__sys:{fileType}`） |
| `typeFilters` | 逗号分隔或数组；匹配 `assets.type_id`（`__sys:image` 或用户 uuid）；多个为 **OR** |
| `categories` | **兼容**；并入 `typeFilters` |
...
| `extension` | 按扩展名精确筛选（不含点，如 `png`；带点会自动去掉） |
...
按用户分类或系统格式筛选（`typeFilters` 多项为 OR）：

```bash
curl -s "http://127.0.0.1:41596/api/v1/asset/get?typeFilters=__sys:image,分类UUID&limit=50"
```

按扩展名精确筛选（与列表视图「扩展名」列一致）：

```bash
curl -s "http://127.0.0.1:41596/api/v1/asset/get?extension=png&limit=50"
```

列出当前资料库已入库的扩展名（按资产数量降序，值为不含点的扩展名）：

```bash
curl -s "http://127.0.0.1:41596/api/v1/asset/extensions"
```
...
curl -s -X POST http://127.0.0.1:41596/api/v1/category/assign \
  -H "Content-Type: application/json" \
  -d "{\"assetIds\":[\"上一步返回的 assetId\"],\"typeId\":\"用户分类UUID\"}"
```
...
# 类型列表（含系统 __sys:* 与用户分类）
curl -s http://127.0.0.1:41596/api/v1/category/get
...
# 创建用户分类并关联资产
curl -s -X POST http://127.0.0.1:41596/api/v1/category/create \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"待整理\",\"color\":\"#f59e0b\"}"

curl -s -X POST http://127.0.0.1:41596/api/v1/category/assign \
  -H "Content-Type: application/json" \
  -d "{\"assetIds\":[\"资产UUID\"],\"typeId\":\"__sys:image\"}"
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `sourceLibraryRoot` | 是 | 源资料库根目录（与当前库同为 archive 或 catalog） |

**Response `data`：** `ImportLibrarySuccess`（含 `categoriesCreated` / `categoriesMerged`；见 [§5.2.1](#521-从其它资料库合并导入) 与 OpenAPI `ImportLibrarySuccess`）。
...
> 不支持 `tagIds` / `categoryIds`；见 [§3.7](#37-单资产导入与元数据标签--类型)。
...
| `SYSTEM_CATEGORY_IMMUTABLE` | 400 | 系统类型（`__sys:*`）不可创建/修改/删除 |
...
- 标签与类型须导入后分别调用 `POST /tag/assign`、`POST /category/assign`（[§3.7](#37-单资产导入与元数据标签--类型)）
- 资料库整库合并：`POST /library/importFromLibrary`（自动迁移标签与 `type_id`）
