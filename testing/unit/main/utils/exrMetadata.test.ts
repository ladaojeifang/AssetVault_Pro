...
describe('walkExrAttributes longNames fallback', () => {
  it('parses null-terminated attrs when LONG_NAMES is set but names are short', () => {
    const buf = buildMinimalExrHeaderBuffer(EXR_FLAG_LONG_NAMES)
    const attrs = walkExrAttributes(buf)
    expect(attrs.some((a) => a.name === 'dataWindow')).toBe(true)
    expect(attrs.some((a) => a.name === 'channels')).toBe(true)
  })
})
