...
Product roadmaps and commercial plans live in **`doc-internal/`** (local only, not on GitHub). Forks may redistribute under MIT; see [TRADEMARK.md](TRADEMARK.md) for naming and logo rules.
...
```bash
pnpm run typecheck
pnpm run test          # unit + gen (CI)
pnpm run test:all      # includes integration (local)
```

See [testing/README.md](testing/README.md). Manual UI regression checklists are in **`doc-internal/regression/`** (local only).
