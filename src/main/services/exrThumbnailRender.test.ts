...
import { pickExrThumbnailLayerName } from './exrExrsDecoder'
...
describe('renderExrThumbnailWebp', () => {
  it('falls back to napi/ffmpeg when exrs fails', async () => {
    ...
    vi.doMock('../utils/exrRaster', () => ({
      rasterizeExrToWebpBuffer: mockRaster
    }))

    vi.resetModules()
    const { renderExrThumbnailWebp } = await import('./exrThumbnailRender')
