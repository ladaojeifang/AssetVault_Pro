...
import { readLibraryDisplayName } from './libraryManifest'
...
describe('readLibraryDisplayName', () => {
  ...
  it('derives embedded infra subfolder label from ParentName-uuid pattern', () => {
    root = mkdtempSync(join(tmpdir(), 'av-lib-label-'))
    writeManifest({ displayName: 'AssetVault Library', libraryMode: 'embedded' })
    const embeddedRoot = join(root, 'MyProject-a1b2c3d4')
