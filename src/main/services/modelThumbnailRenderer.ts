...
const HIDDEN_RENDER_TIMEOUT_MS = 90_000
...
function waitForReadySignal(): Promise<void> {
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => reject(new Error('Model thumbnail window init timeout')), 30_000)
    readyResolve = () => {
      clearTimeout(t)
      readyResolve = null
      resolve()
...
async function renderViaMainWindow(absFilePath: string, ext: string): Promise<Buffer | null> {
  ...
  try {
    ...
    const dataUrl = await win.webContents.executeJavaScript(
      `window.__assetVaultRenderModelSnapshot(${JSON.stringify(absFilePath)}, ${JSON.stringify(extClean)}, ${THUMB_PIXEL_SIZE})`
    )
...
async function renderViaHiddenWindow(absFilePath: string, ext: string): Promise<Buffer | null> {
  const win = await ensureThumbnailWindow()
